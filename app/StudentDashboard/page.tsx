...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  const [lifetimeAverageScore, setLifetimeAverageScore] = useState<number | null>(null);
...
const fetchXPDataForMonth = async (year: number, month: number): Promise<XPDataPoint[]> => {
  setIsLoading(true);
  try {
    const { data, error } = await supabase
...
const [attemptedQuizzes, setAttemptedQuizzes] = useState<QuizAttemptWithQuizInfo[]>([]);
...
const fetchAttemptedQuizzes = async () => {
  ...
  try {
    ...
    if (error) {
      ...
      const formattedData: QuizAttemptWithQuizInfo[] = data.reduce((acc: QuizAttemptWithQuizInfo[], attempt: any) => {
        if (
...
{/* ============================================================================================================ */}
  ...
  {/* ============================================================================================================ */}
          ...
          <div className={styles.lifetimeScore}>
            <h1>Average Score</h1>
            {hasQuizAttempts ? (
              <h2>{lifetimeAverageScore !== null ? lifetimeAverageScore.toFixed(2) : '...'}%</h2>
            ) : (
...
<div className={styles.graphContainer}>
  <Card style={{backgroundColor: '#7FAFA0', border: '2px solid lightblue' }}>
    <CardHeader>
      ...
      <CardDescription style={{color: '#F6F8D5'}}>Your Journey ({currentMonth})</CardDescription>
    </CardHeader>
    <CardContent>
      {isLoading ? (
        ...
        <ResponsiveContainer width="100%" height={142}>
          ...
          >
            ...
            <ChartTooltip
              cursor={false}
              contentStyle={{ backgroundColor: "#456A66", color: "#fff", borderColor: "#97D6DD", borderRadius: "13px" }}
    ...
    <CardFooter className="flex-col items-start gap-2 text-sm">
      ...
      <div className="leading-none text-muted-foreground text-[#C1C2AA]" >
          ...
          attemptedQuizzes.map((attempt) => (
            ...
            >
              <div className={styles.namePercentage}>
                ...
                <p>{attempt.correct_questions} / {attempt.total_questions}</p>
              </div>

              <div className={styles.desCode}>
                <p>{attempt.quiz.description}</p>
                <Badge variant="outline" className={styles.joinCode}>{attempt.quiz.join_code}</Badge>
              </div>
              ...
              <div className={styles.startComplete}>
                ...
                <img className={styles.startCompleteImg}
                ...
                <div className={styles.completeDate}>
                  <h1>Complete</h1>
                  <p>{attempt.completed_at ? new Date(attempt.completed_at).toLocaleDateString() : 'Not Completed Yet !'}</p>
                </div>
              </div>
            </div>
          ))
