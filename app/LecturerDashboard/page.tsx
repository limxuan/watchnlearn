...
//Get user Name, profile picture, Role///////////////////////////////////////////////////////
  ...
  const userId = "c7b2a173-4041-486a-a279-21158daa3bfe"; // Replace this with a real ID from your DB
  //77cea966-8837-4831-93a7-12b1f74e7293
  //c7b2a173-4041-486a-a279-21158daa3bfe
  //6f47bc87-ba30-4f85-94da-2755ed00eefc
...
const [userBadgesInfo, setUserBadgesInfo] = useState<{ url: string; name: string }[]>([]);
...
const handleEditProfile = async () => {
  if (newPasswordInput && !isCurrentPasswordCorrect) {
    alert('Please enter the correct current password to update your password.');
    return;
  }
  ...
  const updates: { username?: string; pfp_url?: string; email?: string; password?: string } = {};
  if (newUsername !== userInfo?.username) updates.username = newUsername;
  if (newPfpUrl !== userInfo?.pfp_url && newPfpUrl) updates.pfp_url = newPfpUrl; // Only update if new URL exists and is different
...
const [publishedQuizCount, setPublishedQuizCount] = useState<number | null>(null);
const [isLoadingPublishedQuizzes, setIsLoadingPublishedQuizzes] = useState(true);
const [publishedQuizError, setPublishedQuizError] = useState<string | null>(null);
...
const [totalAttemptsAllQuizzes, setTotalAttemptsAllQuizzes] = useState<number>(0)
...
useEffect(() => {
    const fetchQuizAttemptData = async () => {
        try {
            ...
            const quizIdNameMap: { [key: string]: string } = quizzesData.reduce((acc: { [key: string]: string }, quiz) => {
            ...
            attemptsData.forEach((attempt) => {
                quizAttemptCounts[attempt.quiz_id] = (quizAttemptCounts[attempt.quiz_id] || 0) + 1;
            });

            // 4. Format the data for the chart
            const formattedData: QuizAttemptData[] = Object.keys(quizAttemptCounts).map((quizId) => ({
...
interface Quiz {
    ...
    const [deletionNotificationOpen, setDeletionNotificationOpen] = useState(false);
          ...
          if (error) {
            ...
            setPublishedQuizzes((prevQuizzes) =>
              prevQuizzes.map((quiz) =>
                quiz.quiz_id === quizId ? { ...quiz, public_visibility: !isPublic } : quiz
              )
            );
          }
        } catch (err: any) {
            ...
            setPublishedQuizzes((prevQuizzes) =>
              prevQuizzes.map((quiz) =>
                quiz.quiz_id === selectedQuiz.quiz_id
                  ? { ...quiz, name: editQuizName, description: editQuizDescription }
                  : quiz
              )
            );
          ...
          {isLoading ? ( // Show skeleton while loading
            ...
            <div className="flex items-center gap-4 p-2 border-b border-gray-200">
              ...
              <div className="flex items-start space-x-2">
                <div>
                  <p className="font-bold">{userInfo.username || 'No User Found'}</p>
                  {userInfo.role && <Badge className="bg-[#F6F8D5] hover:bg-[#CDCFB2]">{userInfo.role}</Badge>}
                </div>
                <div>
                    ...
                    {userBadgesInfo.length > 0 && (
                      userBadgesInfo.map((badgeInfo, index) => (
                        <div key={index} className="relative w-4 h-4 rounded-full overflow-hidden" >
                          <Image src={badgeInfo.url} alt={`Badge ${index + 1}`} layout="fill" objectFit="cover" title={badgeInfo.name} />
                        </div>
                      ))
                    )}           
                  </Badge>
...
<div className={styles.graphContainer}>
        <Card style={{backgroundColor: '#7FAFA0', border: '2px solid lightblue'}}>
            <CardHeader>
                <CardTitle style={{color: '#6B8474'}}>Quiz Attempt Chart</CardTitle>
                <CardDescription style={{color: '#F6F8D5'}}>Total attempts across all your quizzes</CardDescription>
            </CardHeader>
            
            <CardContent>
                <ChartContainer config={chartConfig} style={{width: "100%", height: "170px" }}>
                    ...
                    <ResponsiveContainer>
                        ...
                        >
                            ...
                            <XAxis dataKey="name" tick={{ fontSize: 10, stroke: "#E8E9CC" }} angle={-30} textAnchor="end" />
            ...
            <CardFooter className="flex-col items-start gap-2 text-sm">
                <div className="leading-none text-muted-foreground text-[#E8E9CC]">
                    Total of {totalAttemptsAllQuizzes} attempts across all your quizzes.
                </div>
            </CardFooter>
        </Card>
...
<div className={styles.recentPublishedQuiz}>
      ...
      <div className={styles.recentPublishedQuizContainer}>
        {isLoading ? (
          ...
          publishedQuizzes.map((quiz) => (
            <div key={quiz.quiz_id} className={styles.singleRecentPublishedQuiz}
                 onMouseEnter={(e) => {
              ...
              <div className={styles.nameEditDelete}>
                ...
                <div className={styles.editDelete}>
                    <Button variant="outline" onClick={() => handleEditClick(quiz)}>
                    Edit
                    </Button>
                    <Button variant="destructive" onClick={() => handleDeleteClick(quiz.quiz_id)}>
                    Delete
                    </Button>
                </div>
              </div>
              ...
              <div className={styles.visibility}>
                ...
                <Switch
                    ...
                    onCheckedChange={(checked) => handleVisibilityChange(quiz.quiz_id, quiz.public_visibility)}
                />
              </div>
            </div>
          ))
      ...
      <Dialog open={editDialogOpen} onOpenChange={setEditDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          ...
          <DialogFooter>
            ...
            <Button type="button" onClick={handleEditSave} style={{ marginBottom: '15px' }}>
              Save changes
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={deleteConfirmationOpen} onOpenChange={setDeleteConfirmationOpen}>
        <DialogContent className="sm:max-w-[425px]">
          ...
          <DialogFooter>
            ...
            <Button type="button" variant="destructive" onClick={confirmDeleteQuiz} style={{ marginBottom: '15px' }}>
              Delete
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={deletionNotificationOpen} onOpenChange={setDeletionNotificationOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
          ...
          <DialogFooter>
            <Button type="button" onClick={() => setDeletionNotificationOpen(false)}>
              OK
            </Button>
          </DialogFooter>
        </DialogContent>
